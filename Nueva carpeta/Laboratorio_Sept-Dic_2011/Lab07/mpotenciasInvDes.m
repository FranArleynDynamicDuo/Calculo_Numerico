% Luis Vieira
% 07-41651
% Funcion que calcula el autovalor mas cercano a alfa y su autovector correspondiente
function [lambda,V] = mpotenciasInvDes(A,alfa,X,epsilon,max1)
% Datos:
%       - A es una matriz de orden NxN y diagonizable
% es decir: P = autovectoresde A, inv(P)
%D = inv(P)*A*P
% 		- alfa es la traslacion
% 		- epsilon es la tolerancia
% 		- max1 es el numero maximo de iteracion
% Resultados:
%       - lambda es el autovalor mas cercano de alfa
%       - V es ekl autovector asociado a lambda


% Inicializacion de los parametros
lambda = 0;
cnt = 0;
err=1;
[n n] = size(A);
A = A - alfa*eye(n);
state = 1;
[L,U] = lu(A);

while((cnt <= max1) && (state ==1))
    %Resolucion del sistema AY=X
    Y = L \(X);
    Y=U\Y;
    %Normalizacion de Y
    [m j] = max(abs(Y));
    c1 = m;
    dc = abs (lambda - c1);
    Y = ( 1/c1)*Y;
    
    % Actualizacion de X y de lambda y criterio de convergencia
    dv = norm( X-Y,inf);
    err = max(dc,dv);
    X = Y;
    lambda = c1;
    state = 0;
    if( err > epsilon)
        state = 1;
    end
    cnt = cnt+1;
end
lambda=alfa+1/lambda;
V = X;